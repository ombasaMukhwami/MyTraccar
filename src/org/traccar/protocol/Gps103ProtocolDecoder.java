...
public class Gps103ProtocolDecoder extends BaseProtocolDecoder {
    ...
    private static final Pattern PATTERN_WLT = new PatternBuilder()
            .text("imei:")
            .number("(d+),")                     // imei
            .expression("([^,]+),")              // alarm
            .groupBegin()
    ...
    private Position decodeWlt(Channel channel, SocketAddress remoteAddress, String sentence) {
        Parser parser = new Parser(PATTERN_WLT, sentence);
        if (!parser.matches()) {
            return null;
        }
       String imei = parser.next();
    ...
    protected Object decode(
        ...
        if (sentence.substring(21, 21 + 2).equals("vr")) {
            ...
            switch (tempArray.length) {
                case 13:
                    return decodeWlt(channel, remoteAddress, sentence);
                 default:
